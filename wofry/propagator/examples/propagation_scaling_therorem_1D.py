...
def figure_of_merit(radius,wavefront,weight_with_intensity=True):
    x = wavefront.get_abscissas()
    new_phase = 1.0 * wavefront.get_wavenumber() * (x**2) / (-2 * radius)

    wavefront2 = wavefront.duplicate()
...
def guess_wavefront_curvature(wavefront,rmin=-10000.0,rmax=10000.0,rpoints=100,plot_scan=True):

    radii = numpy.linspace(rmax,rmin,rpoints)
    ss = numpy.zeros_like(radii)

    for i,radius in enumerate(radii):
        ss[i] =figure_of_merit(radius,wavefront)

    if plot_scan:
        plot(radii,ss)

    res = minimize(figure_of_merit, radii[numpy.argmin(ss)], args=wavefront, method='powell',options={'xtol': 1e-8, 'disp': True})
...
def propagate_wavefront_scaling(wavefront,distance,handler_name=Fresnel1D.HANDLER_NAME,zoom=0.005,
                                using_radius=None):
    ...
    if using_radius is None:
        using_radius = guess_wavefront_curvature(wavefront_inside,rmin=-1000,rmax=1000,rpoints=100,plot_scan=True)

    #
    # make plane wave by adding a spherical wavefront with radius -R
    #
    # wavefront_inside.set_plane_wave_from_complex_amplitude() #!!!!!!!!!!!!!!!!!!!!
    new_phase = 1.0 * wavefront_inside.get_wavenumber() * (wavefront_inside.get_abscissas()**2) / (-2 * using_radius)
    wavefront_inside.add_phase_shifts(new_phase)
    ...
    magnification = (distance + using_radius) / using_radius

    print("R=%f, Delta=%f, M=%f, z=%f"%(using_radius,distance,magnification,distance/magnification))
    ...
    screen = WOScreen(name="PIRRONE")
    coordinates = ElementCoordinates(p = 0.0, q=distance/magnification) #!!!!!!!!!!!!!!!!!!!!
    propagation_elements = PropagationElements()
    propagation_elements.add_beamline_element(BeamlineElement(optical_element=screen,
                                                              coordinates=coordinates))
    ...
    #
    k = output_wf_2.get_wavenumber()

    PHASE1 = numpy.ones(wavefront.get_abscissas().size) * (k * distance * (1 - 1/magnification))
    PHASE2 = k /2 / distance * (magnification - 1)/magnification * (output_wf_2.get_abscissas()*magnification)**2
    PHASE = PHASE2 + PHASE1


    output_wf_3 = GenericWavefront1D.initialize_wavefront_from_arrays(
                    output_wf_2.get_abscissas()*magnification,
                    output_wf_2.get_complex_amplitude()/numpy.sqrt(magnification)*numpy.exp(1j*PHASE),
                    wavelength=output_wf_2.get_wavelength())
...
if __name__ == "__main__":
    ...
    # output_wf_1 = propagate_wavefront(wavefront,distance=5.0,handler_name=Fresnel1D.HANDLER_NAME)
    output_wf_2 = propagate_wavefront(wavefront,distance=distance,handler_name=FresnelZoom1D.HANDLER_NAME,zoom=zoom)
    output_wf_3 = propagate_wavefront_scaling(wavefront,distance=distance,handler_name=FresnelZoom1D.HANDLER_NAME,
                                              zoom=zoom,using_radius=None)

    # plot(output_wf_1.get_abscissas()*1e6, output_wf_1.get_intensity(), title="WF1 - "+Fresnel1D.HANDLER_NAME)
